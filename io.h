...
static void outl(unsigned short port, unsigned int data)
{
	asm volatile ("out %%eax, %%dx\n" :: "a" (data), "d" (port));
}

static void outsl(unsigned short port, const void *buf, size_t len)
{
	asm volatile ("rep;outsl" :: "S" (buf), "c" (len),
			"d" (port) : "cc", "memory");
}

static void insl(unsigned short port, void *buf, size_t len)
{
	asm volatile ("rep;insl" :: "D" (buf), "c" (len),
			"d" (port) : "cc", "memory");
}

static void outsb(unsigned short port, const void *buf, size_t len)
{
	asm volatile ("rep;outsb" :: "S" (buf), "c" (len),
			"d" (port) : "cc", "memory");
}

static void insb(unsigned short port, void *buf, size_t len)
{
